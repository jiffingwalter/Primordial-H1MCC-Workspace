...
(global short testing_delayed 200)
...
;	(if debug (print "save issue debug: game deemed safe to save, attempting save...")) ; DEBUG REMOVE ME
	(game_save_totally_unsafe)
	(set save_now false)
;	(if debug (print "save issue debug: saved sucessfully, setting certain save to FALSE...")) ; DEBUG REMOVE ME
)

; Certain save subroutine
(script static void certain_save
	(set save_now true)
;	(if debug (print "save issue debug: certain save set TRUE...")) ; DEBUG REMOVE ME
)
...
;- Breadcrumbs navpoints ----------------------------------------------------------
(global short g_breadcrumb_nav_index 0)

(script dormant breadcrumbs_nav_points_c20
	; Down hallway as the player is following guilty spark
	(if (= g_breadcrumb_nav_index 0)
		(begin
			(breadcrumbs_activate_team_nav_point_position "default" player 14 13.9 -4.5 "navpoint_1" 0.55)
			(sleep_until
				(or
					(volume_test_objects enc5_trigger (players))
					(> g_breadcrumb_nav_index 0)
				)
			)
			(breadcrumbs_deactivate_team_nav_point player "navpoint_1")
			(set g_breadcrumb_nav_index 1)
		)
	)

	; Fuirther down hall as the player is following guilty spark
	(if (= g_breadcrumb_nav_index 1)
		(begin
			(breadcrumbs_activate_team_nav_point_position "default" player 14.2 -14.2 -4.5 "navpoint_2" 0.55)
			(sleep_until
				(or
					(volume_test_objects enc7_trigger (players))
					(> g_breadcrumb_nav_index 1)
				)
			)
			(breadcrumbs_deactivate_team_nav_point player "navpoint_2")
			(set g_breadcrumb_nav_index 2)
		)
	)

	; Down large hall as the player is following guilty spark
	(if (= g_breadcrumb_nav_index 2)
		(begin
			(breadcrumbs_activate_team_nav_point_position "default" player -8 -31 -4.5 "navpoint_3" 0.55)
			(sleep_until
				(or
					(< (objects_distance_to_position (players) -8 -31 -4.5) 3)
	...
	(if (= g_breadcrumb_nav_index 3)
		(begin
			(breadcrumbs_activate_team_nav_point_position "default" player -41 -74 -4.5 "navpoint_4" 0.55)
			(sleep_until
				(or
					(volume_test_objects enc2_7_trigger (players))
					(> g_breadcrumb_nav_index 3)
				)
			)
	...
	(if (= g_breadcrumb_nav_index 4)
		(begin
			(breadcrumbs_activate_team_nav_point_position "default" player -88.64 -18.35 -4.5 "navpoint_5" 0.55)
			(sleep_until
				(or
					(= (device_group_get platform1) 1)
					(> g_breadcrumb_nav_index 4)
				)
			)
			(breadcrumbs_deactivate_team_nav_point player "navpoint_5")
	...
	(if (= g_breadcrumb_nav_index 5)
		(begin
			(sleep_until
				(or
					(volume_test_objects enc3_1_trigger (players))
					(> g_breadcrumb_nav_index 5)
				)
			)
			(breadcrumbs_activate_team_nav_point_position "default" player -44 -7.5 11.5 "navpoint_6" 0.55)

			(sleep_until
				(or
					(< (objects_distance_to_position (players) -44 -7.5 11.5) 3)
					(> g_breadcrumb_nav_index 5)
				)
			)
	...
	(if (= g_breadcrumb_nav_index 6)
		(begin
			(breadcrumbs_activate_team_nav_point_position "default" player 37 -8 11.5 "navpoint_7" 0.55)
			(sleep_until
				(or
					(= (device_group_get floor2_door2) door_open)
	...
	(if (= g_breadcrumb_nav_index 7)
		(begin
			(breadcrumbs_activate_team_nav_point_position "default" player 54 -21.4 10 "navpoint_8" 0.55)
			(sleep_until
				(or
					(< (objects_distance_to_position (players) 53 -21 10) 3)
	...
	; Down hallway as the player is following guilty spark leading to elevator
	(if (= g_breadcrumb_nav_index 8)
		(begin
			(sleep_until
				(or
					(< (objects_distance_to_position (players) 53.5 -48.6 10) 3)
					(> g_breadcrumb_nav_index 8)
				)
			)
			(breadcrumbs_activate_team_nav_point_position "default" player 80 -76 11.5 "navpoint_9" 0.55)

			(sleep_until
				(or
					(< (objects_distance_to_position (players) 80 -76 12) 5)
					(> g_breadcrumb_nav_index 8)
				)
			)
	...
	(if (= g_breadcrumb_nav_index 9)
		(begin
			(breadcrumbs_activate_team_nav_point_position "default" player 37.87 -83.83 11.5 "navpoint_10" 0.55)
			(sleep_until
				(or
					(= (device_group_get platform2) 1)
	...
	(if (= g_breadcrumb_nav_index 10)
		(begin
			(sleep_until
				(or
					(volume_test_objects enc5_1_trigger (players))
					(> g_breadcrumb_nav_index 10)
				)
			)
			(breadcrumbs_activate_team_nav_point_position "default" player 25 -64.25 26 "navpoint_11" 0.55)

			(sleep_until
				(or
					(< (objects_distance_to_position (players) 25 -63 26) 3)
					(> g_breadcrumb_nav_index 10)
				)
			)
		...
		(begin
			(sleep_until
				(or
					...
					(> g_breadcrumb_nav_index 11)
				)
			)
			(breadcrumbs_activate_team_nav_point_position "default" player -7 -42 27.5 "navpoint_12" 0.55)

			(sleep_until
				(or
					(< (objects_distance_to_position (players) -7 -42 27.5) 3)
					(> g_breadcrumb_nav_index 11)
				)
			)
	...
	(if (= g_breadcrumb_nav_index 12)
		(begin
			(sleep_until
				(or
					(= (device_group_get floor3_door2) door_open)
					(> g_breadcrumb_nav_index 12)
				)
			)
			(breadcrumbs_activate_team_nav_point_position "default" player -38 8 27.5 "navpoint_13" 0.55)

			(sleep_until
				(or
					(< (objects_distance_to_object (players) floor3_door3) 3)
					(> g_breadcrumb_nav_index 12)
				)
			)
	...
	(if (= g_breadcrumb_nav_index 13)
		(begin
			(breadcrumbs_activate_team_nav_point_position "default" player -77 52.38 27.5 "navpoint_14" 0.55)
			(sleep_until
				(or
					(< (objects_distance_to_position (players) -77 52.38 27.5) 3)
					(> g_breadcrumb_nav_index 13)
				)
			)
			(breadcrumbs_deactivate_team_nav_point player "navpoint_14")
	...
	(if (= g_breadcrumb_nav_index 14)
		(begin
			(sleep_until
				(or
					(= (device_group_get floor3_door5) door_open)
					(> g_breadcrumb_nav_index 14)
				)
			)
			(breadcrumbs_activate_team_nav_point_position "default" player -38 72 27.5 "navpoint_15" 0.55)

			(sleep_until
				(or
					(= (device_group_get platform3) 1)
					(> g_breadcrumb_nav_index 14)
				)
			)
			(breadcrumbs_deactivate_team_nav_point player "navpoint_15")
	...
	(if (= g_breadcrumb_nav_index 15)
		(begin
			(sleep_until
				(or
					(volume_test_objects enc7_1b_trigger (players))
					(> g_breadcrumb_nav_index 15)
				)
			)
			(breadcrumbs_activate_team_nav_point_position "default" player -81 61.4 42 "navpoint_16" 0.55)

			(sleep_until
				(or
					(< (objects_distance_to_position (players) -81 61 42) 3)
					(> g_breadcrumb_nav_index 15)
				)
			)
	...
	(if (= g_breadcrumb_nav_index 16)
		(begin
			(breadcrumbs_activate_team_nav_point_position "default" player -78 34.93 43.5 "navpoint_16_5" 0.55)
			(sleep_until
				(or
					(< (objects_distance_to_position (players) -78 34.93 43.5) 3)
					(> g_breadcrumb_nav_index 16)
				)
			)
			(breadcrumbs_deactivate_team_nav_point player "navpoint_16_5")
	...
	(if (= g_breadcrumb_nav_index 17)
		(begin
			(sleep_until
				(or
					(= (device_group_get floor4_door2) door_open)
					(> g_breadcrumb_nav_index 17)
				)
			)
			(breadcrumbs_activate_team_nav_point_position "default" player -41 8 43.5 "navpoint_17" 0.55)

			(sleep_until
				(or
					(< (objects_distance_to_position (players) -41 8 43.5) 3)
					(> g_breadcrumb_nav_index 17)
				)
			)
	...
	(if (= g_breadcrumb_nav_index 18)
		(begin
			(breadcrumbs_activate_team_nav_point_position "default" player 0 0 45 "navpoint_18" 1)
			(sleep_until (> g_breadcrumb_nav_index 18))
...
(global short enc8_4_limiter 0)
(script continuous enc8_4_spawner
	; Sleep until the spawn count is low enough
	(sleep_until 
		(and
			(<= enc8_4_limiter (* 40 spawn_scale))
			(<= (ai_nonswarm_count enc8_4) (* min_combat_spawn 2))
		)
	)
	...
	(if 
		(or 
			(volume_test_objects_all enc8_4_wave_right (players))
			(volume_test_objects_all enc8_4_wave_left (players))
		)
			(begin
				(ai_spawn_actor enc8_4/combats_left)
				(ai_spawn_actor enc8_4/combats_right)
	;			(ai_place enc8_4/infs)
				(set enc8_4_limiter (+ enc8_4_limiter 2))
			)
	)
)
...
(script continuous enc6_4_spawner
	...
	(if 
		...
		;prim; check carrier count then spawn if too low
		(if (< (ai_living_count enc6_4/carriers) min_carrier_spawn)
				; Spawn 'em and increment the count
				(begin 
					(ai_spawn_actor enc6_4/carriers)
					(set enc6_4_limiter (+ enc6_4_limiter 1))
				)
			)
		)
...
(script continuous enc6_2b_spawner
	...
	(if 
		...
		(begin 
			...
			;prim edit; this encounter now spawns one tank, and ONE ONLY!!!!111!!!
			;; Check the carrier form count. If it is too low, spawn
...
(script continuous enc5_5_spawner
	...
	(if 
		...
		;prim edit; adds combat forms to the wave
		; These conditions are met. Spawn.
		(begin
			; Check the carrier form count. If it is too low, spawn
			(if (< (ai_living_count enc5_5/carriers) (* min_carrier_spawn 1))
				; Spawn 'em and increment the count
				(begin 
			...
			;prim; check combat form count, spawn if too low
			(if (< (ai_living_count enc5_5/combats_wave) min_combat_spawn)
				; Spawn 'em and increment the count
				(begin 
					(ai_spawn_actor enc5_5/combats_wave)
					(ai_magically_see_players enc5_5/combats_wave)
					(set enc5_5_limiter (+ enc5_5_limiter 1))
				)
			)
		)
...
; prim ; Encounter 8_skip, triggered by encounter 7_9 if the player goes across the side to avoid battle like a coward
(script dormant enc8_skip
	; Sleep until trigger(s)
	(sleep_until 
		(or
			(volume_test_objects enc8_skip_left (players))
			(volume_test_objects enc8_skip_right (players))
		)
	)
	; Debug
	(if debug (print "Encounter 8 skip punishment..."))
	
	(ai_place enc8_skip)
	(ai_magically_see_players enc8_skip)
	(ai_magically_see_players enc8_3)
)

; prim ; Encounter 8_4
(script dormant enc8_4
	; Sleep until trigger or if 8_3 carriers are almost dead
	(sleep_until 
		(or
			(volume_test_objects enc8_4 (players))
			(< (ai_nonswarm_count enc8_3) 4)
		)
	)
	
	; Debug
	(if debug (print "Encounter 8.4..."))
	(certain_save)
...
; prim ; Encounter 8_3 carriers, triggered by encounter 8_3
(script dormant enc8_3c
	; Sleep until trigger
	(sleep_until (volume_test_objects enc8_3c (players)))
	
	; Debug
	(if debug (print "Encounter 8.3c..."))
	(certain_save)
	
	(ai_place enc8_3/carriers)
	(ai_magically_see_players enc8_3/carriers)
	
	; Sleep the skip punishment encounter
	(sleep -1 enc8_skip)
	
	(wake enc8_4)
)

; prim ; Encounter 8_3, triggered by encounter 8_2b
(script dormant enc8_3
	; Sleep until trigger or 8_2 has been wiped
	(sleep_until 
		(or
			(volume_test_objects enc8_3a (players))
			(< (ai_nonswarm_count enc8_2b) 2)
		)
	)
	
	; Debug
	(if debug (print "Encounter 8.3a..."))
	(certain_save)
	
	(ai_place enc8_3/rockets)
	(ai_set_deaf enc8_3/rockets 1)
	
	; Spawn next wave if 8_3 is almost mopped up or the player crosses the trigger
	(sleep_until 
		(or
			(volume_test_objects enc8_3b (players))
			(< (ai_nonswarm_count enc8_3/rockets) 1)
		)
	)
	; Debug
	(if debug (print "Encounter 8.3b..."))
	(certain_save)
	
	(ai_place enc8_3/combats)
	(ai_magically_see_players enc8_3)
	
	(wake enc8_3c)
)

; prim ; Encounter 8_2, triggered by encounter 8_1
(script dormant enc8_2
	; Sleep until trigger
	(sleep_until (volume_test_objects enc8_2a (players)))
	
	; Debug
	(if debug (print "Encounter 8.2a..."))
	(certain_save)
	
	(ai_place enc8_2a)
	(ai_set_deaf enc8_2a 1)
	
	; Spawn next wave if 8_2 is almost mopped up or the player crosses the trigger
	(sleep_until (volume_test_objects enc8_2b (players)))
	
	; Debug
	(if debug (print "Encounter 8.2b..."))
	
	(ai_place enc8_2b)
	(ai_magically_see_players enc8_2b)
	
	(wake enc8_3)
)

; prim ; Encounter 8_1, triggered by encounter 7_9
(script dormant enc8_1
	; Sleep until trigger
	(sleep_until (volume_test_objects enc8_1 (players)))
	
	; Debug
	(if debug (print "Encounter 8.1..."))
	(certain_save)
	
	(ai_place enc8_1)
	(ai_magically_see_players enc8_1)
...
(script dormant enc7_12
	...
	; music final start
	(if debug (print "Music final: start"))
	(sound_looping_start "swfce\levels\c20\music\c20_finalbattle" none 1)
	...
	; Debug
	(if debug (print "Section 8..."))
	(certain_save)
)

;prim; extra encounter 7_sanic, triggered by 7_9
(script dormant enc7_sanic
	(if debug (print "Encounter Sanic awoken..."))
	; sleep until player(s) trigger trigger
	(sleep_until (volume_test_objects enc7_sanic_trigger (players)))
	
	; Debug
	(if debug (print "Encounter Sanic triggered, waiting a second..."))
	
	; wait a bit...
	(sleep 500)
	
	; spawn a sanic and give it magic sight
	(ai_place enc7_sanic)
	(ai_magically_see_players enc7_sanic)
	; Debug
	(if debug (print "SANIC SPAWNED!"))
)
...
;	(sleep_until (<= (ai_nonswarm_count enc7_12) 0) 30 1800)
	...
	(object_destroy armin)
	
	; Invincible players
	(object_cannot_take_damage (players))

	; music 05 end
	(if debug (print "Music final: stop"))
	(sound_looping_stop "swfce\levels\c20\music\c20_finalbattle")

   (if (mcc_mission_segment "cine2_final") (sleep 1))

	(set g_breadcrumb_nav_index 19)

	; Fin, decide what cutscene to play based on difficulty
	(if (= "easy" (game_difficulty_get_real))
		(cutscene_extraction_easy)
	)
	(if (= "normal" (game_difficulty_get_real))
		(cutscene_extraction_normal)
	)
	(if 
		(or
...
(script dormant enc7_9
	...
	; MUZAK
	;(sound_looping_set_alternate "swfce\levels\c20\music\c20_04" true)
...
(script dormant enc7_3
	...
	;prim; place rats
	(ai_place bsp3_rats/rats7_3)
	
	; Wake the spawner -- prim says do not!
	;(wake enc7_3_spawner)
	
	; spawn enc 7_3, wait a sec then spawn hammer boi
	(ai_place enc7_3/combats)
	(ai_magically_see_players enc7_3/combats)
	
	(sleep 100)
	(ai_place enc7_3/hammertime)
	(ai_magically_see_players enc7_3/hammertime)
)
...
(script dormant section7
	...
	; music 04 start
	(if debug (print "Music 04: start"))
	(sound_looping_start "swfce\levels\c20\music\c20_04" none 1)
	
	; clean up dancing egg assuming it was triggered
	(object_destroy disco_floor)
	(object_destroy disco_light_large)
	(object_destroy disco_confetti)
	(object_destroy song_betteroffalone)
	(ai_erase party_rats)
	(sleep -1 enc_dance_egg)
	...
	;prim; wake voluntary sanic encounter
	(wake enc7_sanic)
	
	; Wake the monitor
	(wake monitor4_1)
	
	; Spawn camera if cammode is on
	(if camera_mode (object_create camera4))
)
...
(script dormant enc6_8
	...
	; music amogus stop
	(if debug (print "Music amogus: stop"))
	(sound_looping_stop "swfce\levels\c20\music\c20_amogus")
...
(script dormant enc6_4
	...
	; music amogus start
	(if debug (print "Music amogus: start"))
	(sound_looping_start "swfce\levels\c20\music\c20_amogus" none 1)
...
(script dormant enc6_3
	...
	; music 03 stop
	(sleep_until (volume_test_objects enc6_3_end_music_trigger (players)))
	(if debug (print "Music 03: stop"))
...
(script dormant enc6_2
	...
	;prim; sleep for a sec after waking the waves, then spawn a tank
	(sleep 120)
	(ai_spawn_actor enc6_2b/carriers)
)
...
;	(certain_save)
	...
	; if egg was triggered, turn music back on and reset rat alliegence
	(if dance_egg_triggered (print "Dance egg was triggered, restarting music 03..."))
	(if dance_egg_triggered (sound_looping_start "swfce\levels\c20\music\c20_03" none 1))
	(ai_allegiance unused6 player)
...
; dancing easter egg triggered by player finding hidden area, triggered by enc 5_6
(global boolean dance_egg_triggered false)
(script dormant enc_dance_egg
	; kill music if player enters first trigger
	(sleep_until (volume_test_objects dance_kill_music (players)))
	(set dance_egg_triggered true)
	(kill_all_music)

	; wait for player to enter main trigger
	(if debug (print "Entering dancing trigger zone, killing music"))
	(sleep_until (volume_test_objects dance_trigger (players)))
	
	(if debug (print "Dance egg triggered, spawning stuff..."))
	(print "Secret discovered!") ; don't think this will work in retail but would be funny if it does
	
	; spawn dancing stuff
	(ai_place party_rats) ; party rockers in tha hooouse tooniiiight
	(object_create disco_light_large)
	(object_create disco_floor)
	(object_create disco_confetti)
	(object_create song_betteroffalone)
	
	; if player kills the king, sick everyone on the player
	(sleep_until (< (ai_strength party_rats/ratking) .1))
	
	(if debug (print "Wrath of the King Rat has been incurred"))
	
	(object_destroy song_betteroffalone)
	(effect_new "swfce\sound\sfx\impulse\record_scratch\record scratch sfx" record_scratch_sfx)
	
	(ai_set_team party_rats flood)
	(ai_allegiance_remove unused6 player)
	(ai_command_list_advance party_rats)
	(ai_magically_see_players party_rats)
)
...
(script dormant floor3_door2
	...
	(sleep -1 enc5_5_spawner)
	(sleep 900)
	;(sound_looping_start "swfce\levels\c20\music\c20_01" none 1)
)
...
(script dormant enc5_5
	...
	;prim; place 5_5 rats
	(ai_place bsp2_rats/rats5_5)
	
	; Begin the spawner
	(wake enc5_5_spawner)
	
	; MUZAK
	;(sound_looping_set_alternate "swfce\levels\c20\music\c20_03" true)
...
(script dormant enc5_1
	...
	;prim; place semi hidden grabblecarrier
	(ai_place enc5_1b/carrier_grabble)
	(ai_set_deaf enc5_1b/carrier_grabble 1)
	...
	;prim; place 5_1 rats
	(ai_place bsp2_rats/rats5_1)
)

;prim; morshu egg
(script dormant enc_morshu
	(sleep_until (volume_test_objects morshu_spawn (players)) testing_fast)
	(if debug (print "Morshu egg - placed Morshu"))
	(ai_place morshu)
	
	(sleep_until (volume_test_objects morshu_talk (players)) testing_fast)
	(if debug (print "Morshu egg - Morshu intro line"))
	(if (> (ai_strength morshu) 0.1) (sound_impulse_start "swfce\sound\dialog\morshu\morshu intro" (list_get (ai_actors morshu) 0) 1))
	(sleep (max 0 (- (sound_impulse_time "swfce\sound\dialog\morshu\morshu intro") 60)))
	(if (> (ai_strength morshu) 0.1) (object_create morshu_control))
	
	(sleep_until (< 0 (device_group_get morshu_deal)) 1)
	(if debug (print "Morshu egg - Morshu refusal line"))
	(if (> (ai_strength morshu) 0.1) (sound_impulse_start "swfce\sound\dialog\morshu\morshu refuse sale" (list_get (ai_actors morshu) 0) 1))
	(if (> (ai_strength morshu) 0.1) (object_set_permutation (list_get (ai_actors morshu) 0) "__unnamed" frame2))
	(object_destroy morshu_control)
)
...
(script dormant section5
	...
	; music 03 start
	(if debug (print "Music 03: start"))
	(sound_looping_start "swfce\levels\c20\music\c20_03" none 1)
...
;	(object_create scen5_17)
	...
	; Spawn camera if cammode is on
	(if camera_mode (object_create camera3))
...
;	(certain_save)
	...
	;prim; place rats after trigger
	(sleep_until (volume_test_objects enc4_7_rats (players)))
	(ai_place bsp1_rats/rats4_7)
...
(script dormant enc4_5
	...
	(ai_place enc4_5/carrier_grabble)
	(ai_set_deaf enc4_5/carrier_grabble 1)
)
...
(script dormant enc4_0
	; Sleep until the trigger
	(sleep_until (volume_test_objects enc4_0_gun (players)))
...
(script dormant floor2_door1
	;prim; Sleep for a while, then spawn a plasma cannon gunner
	(sleep 600)
	(ai_place enc3_4/runners)
	;prim; sleep awhile longer
	(sleep 600)
...
(script dormant enc3_4
	...
	; Wake the wave, spawn a gunner
	(wake enc3_4_spawner)
	
	; MUZAK
	;(sound_looping_set_alternate "swfce\levels\c20\music\c20_02" true)
...
(script dormant enc3_1
	...
	; music 02 start
	(if debug (print "Music 02: start"))
	(sound_looping_start "swfce\levels\c20\music\c20_02" none 1)
...
;	(object_destroy scen2_1)
	...
	; Spawn camera if cammode is on
	(if camera_mode (object_create camera2))
)
...
;	(sleep_until (<= (ai_living_count enc2_11) 1))
	...
	; Sleep until the flood are dead or player got to the elevator
	(sleep_until
		(or
			(= (ai_nonswarm_count enc2_12b) 0)
			(volume_test_objects tv_platform1 (players))
		)
	)
	
	(if debug (print "Music 01: stop"))
	(sound_looping_stop "swfce\levels\c20\music\c20_01")
...
;	(certain_save)
	...
	;prim; place hidden keyes dorito easter egg
	(ai_place enc2_10b)
)
...
(script dormant enc8
	...
	;prim; place first ambient rats
	(ai_place bsp0_rats/rats0)
...
(script dormant enc3
	...
	; MUZAK
	;(sound_looping_set_alternate "swfce\levels\c20\music\c20_01" false)
)
...
(script dormant enc1
	...
	; Fire up the music
	;(sound_looping_set_alternate "swfce\levels\c20\music\c20_01" true)
...
(script dormant cameramode_trigger
	...
	(object_create camera1)
	
	; spawn cheat buttons for testing inside MCC
	(wake dev_cheats)
	
	; turn bump possession off when player leaves trigger area
	;(sleep_until (volume_test_objects enc1 (players)))
	;(print "A player left the area, turning bump possession OFF...")
...
;- Encounter skips
(script static void skipto_enc3_4 ; locked door waves on second level
	(if debug (print "Skipping to encounter 3.4..."))
	(kill_all_music)
	(switch_bsp 1)
	(volume_teleport_players_not_inside enc3_4_trigger tp_enc3_4)
	(wake enc3_4)
)

(script static void skipto_enc5_5 ; locked door waves on third level
	(if debug (print "Skipping to encounter 5.5..."))
	(kill_all_music)
	(switch_bsp 2)
	(volume_teleport_players_not_inside enc5_5_trigger tp_enc5_5)
	(wake enc5_5)
)

(script static void skipto_enc6_4 ; locked room with amogus waves
	(if debug (print "Skipping to encounter 6.4..."))
	(kill_all_music)
	(switch_bsp 2)
	(volume_teleport_players_not_inside enc6_4b_trigger tp_enc6_4)
	(wake enc6_4)
)

(script static void skipto_enc7_9 ; 4th floor, pre boss room
	(if debug (print "Skipping to encounter 7.9..."))
	(kill_all_music)
	(switch_bsp 3)
	(volume_teleport_players_not_inside enc7_9_trigger test)
	(wake enc7_9)
)

(script static void skipto_enc8_4 ; 4th floor, pre final waves
	(if debug (print "Skipping to encounter 8.4..."))
	(kill_all_music)
	(switch_bsp 3)
	(volume_teleport_players_not_inside enc8_4 tp_enc8_4)
	(wake enc7_10)
	(wake enc8_4)
)
...
(script static void skipto_s3 ; after first elevator
	(if debug (print "Skipping to section 3..."))
	(kill_all_music)
	(switch_bsp 1)
...
(script static void skipto_s4
	(if debug (print "Skipping to section 4..."))
	(kill_all_music)
	(switch_bsp 1)
...
(script static void skipto_s5 ; after second elevator
	(if debug (print "Skipping to section 5..."))
	(kill_all_music)
	(switch_bsp 2)
...
(script static void skipto_s6 ; after 3rd floor exterior 
	(if debug (print "Skipping to section 6..."))
	(kill_all_music)
	(switch_bsp 2)
...
(script static void skipto_s7 ; after fourth elevator
	(if debug (print "Skipping to section 7..."))
	(kill_all_music)
	(switch_bsp 3)
	(wake enc7_1)
...
(script static void kill_all_music
	(if debug (print "Stopping all music..."))
	(sound_looping_stop "swfce\levels\c20\music\c20_01")
	(sound_looping_stop "swfce\levels\c20\music\c20_02")
	(sound_looping_stop "swfce\levels\c20\music\c20_03")
	(sound_looping_stop "swfce\levels\c20\music\c20_04")
	(sound_looping_stop "swfce\levels\c20\music\c20_05")
	(sound_looping_stop "swfce\levels\c20\music\c20_amogus")
	(sound_looping_stop "swfce\levels\c20\music\c20_finalbattle")
)

(script static void cheats_on
	(print "Turning ON cheats!")
...
(script dormant dev_cheats
	(sleep -1 cheat_control)
	(sleep -1 cheat_allweapons_control)

	(object_create control_cheats)
	(object_create control_sec3)
	(object_create control_sec5)
	(object_create control_sec7)
	(object_create control_enc7_9)
	(object_create control_enc8_4)
	(object_create control_allweapons)
	(wake cheat_control)
	(wake sec3_skip_control)
	(wake sec5_skip_control)
	(wake sec7_skip_control)
	(wake enc7_9_skip_control)
	(wake enc8_4_skip_control)
	(wake cheat_allweapons_control)
)

(script continuous cheat_control
	; panel turned ON
	(sleep_until (= 1 (device_get_position control_cheats)) 1)
	(cheats_on)
	
	; panel turned OFF
	(sleep_until (= 0 (device_get_position control_cheats)) 1)
	(cheats_off)
	; REPEAT...
)

(script dormant sec3_skip_control
	(sleep_until (< 0 (device_get_position control_sec3)) 1)
	(skipto_s3)
)

(script dormant sec5_skip_control
	(sleep_until (< 0 (device_get_position control_sec5)) 1)
	(skipto_s5)
)

(script dormant sec7_skip_control
	(sleep_until (< 0 (device_get_position control_sec7)) 1)
	(skipto_s7)
)

(script dormant enc7_9_skip_control
	(sleep_until (< 0 (device_get_position control_enc7_9)) 1)
	(if debug (print "Music 04: start -- skip only"))
	(sound_looping_start "swfce\levels\c20\music\c20_04" none 1)
	(skipto_enc7_9)
)

(script dormant enc8_4_skip_control
	(sleep_until (< 0 (device_get_position control_enc8_4)) 1)
	(skipto_enc8_4)
)
...
(script continuous cheat_allweapons_control
	...
	(cheat_all_weapons)
	(if debug (print "dropping weapons! (device turned on)"))
	
	; panel turned OFF
	(sleep_until (= 0 (device_get_position control_allweapons)) 1)
	(cheat_all_weapons)
	(if debug (print "dropping weapons! (device turned off)"))
	; REPEAT...
)
...
(script startup mission
	; auto turn on debug ; DEV ONLY
	(set debug false)
	
	; Fade out
	(fade_out 0 0 0 0)

	;prim; turn on cheats automatically if debug is on at the beginning of the level
	(if debug (print "Debug was on at the start of the level..."))
	(if debug (cheats_on))
	...
	;prim; set normal rats to be friendly to all :)
	(ai_allegiance human unused6)
	(ai_allegiance flood unused6)
	(ai_allegiance sentinel unused6)
   ...
   (mcc_mission_segment "01_start")
	...
	(if (breadcrumbs_nav_points_active)
		(wake breadcrumbs_nav_points_c20)
	)
)
