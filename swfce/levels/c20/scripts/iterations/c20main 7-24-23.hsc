...
(script dormant enc7_12
	...
	; music final start
	(if debug (print "Music 05: start"))
	(sound_looping_start "swfce\levels\c20\music\c20_05" none 1)
...
;	(sleep_until (<= (ai_living_count enc2_11) 1))
	...
	;prim; sleep for a sec then turn off music
	(sleep 120)
...
(script static void skipto_s5egg ; after 3rd floor exterior 
...
(script startup mission
	; auto turn on debug ; DEBUG REMOVE ME
	(set debug true)
	...
	(if debug (print "Debug was on at the start of the level, turning on cheats..."))
	(if debug (set cheat_deathless_player true))
	(if debug (set cheat_bottomless_clip true))
