...
;prim; are we testing spawn encounters? (assume player is in encounter's trigger volume)
(global boolean enc_testing false)
...
(script continuous enc6_4_spawner
	...
	(if 
		...
		(if (< (ai_living_count enc6_4/combats) (* min_combat_spawn 2))
			...
			;prim; spawn a heavy if higher difficulty
			(ai_spawn_actor enc6_4/combats_heavy)
		)
...
; ENCOUNTER 5_5 SPAWN TEST ---------------------
(script continuous enc5_5_spawner_test
	; Check if the player is in the spawn area and the limit isn't exceeded
	(if enc_testing
		;(and 
		...
		(begin
			...
			(if (< (ai_living_count enc5_5/carriers) (* min_carrier_spawn 1))
				; Spawn 'em and increment the count
				(begin 
					(if debug (print "enc5_5: attempting carrier spawn..."))
...
(script dormant enc5_5_test
	; Debug
	(if debug (print "Encounter 5.5 TEST"))
	
	; Place the units
	(ai_place enc5_5/combats)
	
	; Begin the spawner
	(wake enc5_5_spawner_test)
)
;5_5 TEST ^------------------------------
